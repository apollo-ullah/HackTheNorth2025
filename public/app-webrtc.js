class StacyWebRTCClient {
        ...
        try {
            ...
            switch (name) {
                ...
                case 'call_police_vapi':
                    result = await this.callPoliceVAPI(args);
                    break;
                default:
    ...
    async callPoliceVAPI(args) {
        try {
            // Use user's actual phone number if not provided
            const userPhone = args.user_phone || '+15146605707';

            const response = await fetch('/api/emergency/vapi-test', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    sessionId: this.sessionId,
                    userName: args.user_name || 'User',
                    userPhone: userPhone,
                    emergencyType: args.emergency_type || 'Emergency'
                })
            });
            const data = await response.json();

            // Show VAPI emergency call status
            if (data.success) {
                this.addMessage('system', `🚨 VAPI emergency call initiated: ${data.call_id}`);
                this.addMessage('ai', 'I\'ve called the dispatcher and am briefing them on your situation. Please wait while I prepare to connect you.');

                // Show connect button after a delay to allow briefing
                setTimeout(() => {
                    this.showConnectToDispatcherButton(data.call_id, userPhone);
                }, 10000); // 10 second delay for briefing

                return { success: true, call_id: data.call_id, message: 'VAPI emergency call initiated' };
            } else {
                this.addMessage('system', `❌ VAPI emergency call failed: ${data.error}`);
                return { error: data.error };
            }

        } catch (error) {
            console.error('VAPI emergency call error:', error);
            this.addMessage('system', `❌ VAPI emergency call error: ${error.message}`);
            return { error: error.message };
        }
    }

    showConnectToDispatcherButton(callId, userPhone) {
        const buttonHtml = `
            <div class="emergency-connect-section" style="margin: 10px 0; padding: 15px; background: #fff3cd; border: 1px solid #ffeaa7; border-radius: 8px;">
                <p style="margin: 0 0 10px 0; font-weight: bold; color: #856404;">
                    📞 Dispatcher has been briefed and is ready to speak with you
                </p>
                <button id="connectToDispatcher" class="emergency-button" style="background: #28a745; color: white; border: none; padding: 10px 20px; border-radius: 5px; cursor: pointer; font-weight: bold;">
                    🔗 Connect Me to Dispatcher
                </button>
            </div>
        `;

        this.addMessage('system', buttonHtml);

        // Add click handler
        setTimeout(() => {
            const button = document.getElementById('connectToDispatcher');
            if (button) {
                button.addEventListener('click', () => {
                    this.connectToDispatcher(callId, userPhone);
                });
            }
        }, 100);
    }

    async connectToDispatcher(callId, userPhone) {
        try {
            this.addMessage('system', '📞 Connecting you to the dispatcher...');

            const response = await fetch('/api/emergency/connect-user', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    sessionId: this.sessionId,
            ...
            if (data.success) {
                this.addMessage('system', `✅ Connection initiated! You should receive a call shortly at ${userPhone}`);
                this.addMessage('ai', 'You\'re now being connected to the dispatcher. Please answer your phone and speak directly with them.');
            } else {
                this.addMessage('system', `❌ Connection failed: ${data.error}`);
            }

        } catch (error) {
            console.error('Connect to dispatcher error:', error);
